...
| Role links per server                 | 3         | Up to 211  |
...
Free plan: 100 users per role link. Premium: up to 30,000,000 users per role link. The user limit applies per individual role link, not per server. The number of role links shares the server's integration quota: 3 free plus any paid slots assigned to that server.
